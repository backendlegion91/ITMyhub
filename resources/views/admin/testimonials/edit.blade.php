...
@section('content')
  ...
  <div class="max-w-3xl mx-auto bg-white shadow-md rounded-lg p-6">
    <h2 class="text-2xl font-semibold text-gray-800 mb-6">Edit Testimonial</h2>
    ...
    <form action="{{ route('admin.testimonials.update', $testimonial->id) }}" method="POST" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="role" id="role" required value="{{ old('role', $testimonial->role) }}"
               class="w-full mt-1 p-3 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500" />
      </div>
      ...
      <div>
        ...
        <input type="url" name="linkedin" id="linkedin" value="{{ old('linkedin', $testimonial->linkedin) }}"
               class="w-full mt-1 p-3 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500" />
      </div>
      ...
      <div>
        ...
        <textarea name="message" id="message" rows="5" required
                  class="w-full mt-1 p-3 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500">{{ old('message', $testimonial->message) }}</textarea>
      </div>

      <div class="pt-4 flex justify-between">
        <a href="{{ route('admin.testimonials.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded hover:bg-gray-300">Back</a>
        <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Update Testimonial</button>
      </div>
    </form>
  </div>
